...
<?php
  if(isset($_POST['add'])){
   ...
   if(empty($errors)){
           $Nombre = remove_junk($db->escape($_POST['NombreRol']));
          $cargo = remove_junk($db->escape($_POST['NombreRol']));
         $estado = remove_junk($db->escape($_POST['estado']));

        $query  = "INSERT INTO user_groups (";
        $query .="NombreRol,NombreRol,estado";
        $query .=") VALUES (";
        $query .=" '{$name}', '{$cargo}','{$estado}'";
        $query .=")";
        if($db->query($query)){
          //sucess
...
<div class="login-page">
    <div class="text-center">
       <h3>Agregar nuevo tipo de usuarios</h3>
     </div>
